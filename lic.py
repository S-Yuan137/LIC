...
def LIC3d(vectorfield, length):
    np.random.seed(10)
    input_texture_arr = np.random.randint(0,2,size = vectorfield.size)
    input_texture = ti.var(ti.f64,  vectorfield.size)
    input_texture.from_numpy(input_texture_arr)
    # output_texture = np.zeros(vectorfield.size)
    output_texture = ti.var(dt=ti.f64, shape= vectorfield.size)
    ...
    @ti.kernel
    def fill_3d():
        for i, j, k in ti.ndrange(vectorfield.size[0], vectorfield.size[1], vectorfield.size[2]):
            output_texture[i,j,k] = LIC_singleLine(input_texture, vectorfield.streamline((i,j,k), length))
    fill_3d()
